...
public class Build extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btnParSeq.getText().equals(sequential)){
			...
			TaskInfoNoArgs<Void> task = (TaskInfoNoArgs<Void>) ParaTask.asTask(()->houseApplet.buildSingleTask(colorWalls, colorRoof));
			ParaTask.registerSlotToNotify(task, ()->finishedBuilding());
			task.start();
		}
		else{
			currentJob = "Parallel";
			...
			TaskInfoNoArgs<Void> task = (TaskInfoNoArgs<Void>) ParaTask.asTask(()->houseApplet.buildTask(colorWalls, colorRoof));
			ParaTask.registerSlotToNotify(task, ()->finishedBuilding());
			task.start();
		}
		//btnBuild.setEnabled(true);
