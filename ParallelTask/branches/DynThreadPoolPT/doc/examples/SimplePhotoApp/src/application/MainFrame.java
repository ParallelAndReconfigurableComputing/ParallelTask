...
import paratask.runtime.ParaTask.ScheduleType;
...
//		newImageProject();
	...
	private Action actionShowAbout = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			JOptionPane.showMessageDialog(MainFrame.this, label, "About "+applicationName, JOptionPane.PLAIN_MESSAGE, new ImageIcon(appLogo)); 
		}
	};
	...
	private void showFutureWork() {
		...
		JOptionPane.showMessageDialog(MainFrame.this, label, "Future release", JOptionPane.PLAIN_MESSAGE, new ImageIcon("images/tools.png"));
	}
	...
	private void checkExit() {
		for (int i = 0; i < pane.getTabCount(); i++) {
			...
			if (!project.isSaved() || project.isModified()) {
				int resp = JOptionPane.showConfirmDialog(MainFrame.this, "There are unsaved projects open. Are you sure you want to exit?", "Exit confirmation", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE );
...
//			actionSaveCurrentProject.setEnabled(!project.isSaved() || project.isModified());
	...
	private void closeCurrentProject() {
		...
		if (comp instanceof ProjectPanel) {
			ProjectPanel projectPanel = (ProjectPanel) comp;
			if (!projectPanel.isSaved()) {
				int resp = JOptionPane.showConfirmDialog(MainFrame.this, "Would you like to save this project before closing?", "Close confirmation", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE );
				if (resp == JOptionPane.YES_OPTION) {
				...
				int resp = JOptionPane.showConfirmDialog(MainFrame.this, "Save modifications before closing?", "Close confirmation", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE );
				if (resp == JOptionPane.YES_OPTION) {
					boolean saved = saveCurrentProject();
					if (saved)
						pane.remove(comp);
