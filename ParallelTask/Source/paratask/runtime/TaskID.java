...
import paratask.logger.TaskLogger;
import paratask.queues.PipelineQueue;
...
public class TaskID<E> {
	...
	protected Integer parentTask=null;
	...
	TaskID(TaskInfo taskInfo) {
		...
		// Task Execution Logging code: start
		long currentThread = Thread.currentThread().getId();
		parentTask = TaskLogger.threadTaskMap.get(currentThread);
		if (TaskLogger.debugFlag && TaskLogger.debugFlagMain) {

			TaskLogger.TaskDetailsArray.add(new StringBuffer("TaskID:"
					+ globalID + ", created: " + System.currentTimeMillis()
					+ ", creating_thread: " + Thread.currentThread().getName()
					+ ", parent_task: " + parentTask));
		}
		// Task Execution Logging code: end
