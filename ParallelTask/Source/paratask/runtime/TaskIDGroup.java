...
public class TaskIDGroup<E> extends TaskID<E> {
	...
	public TaskIDGroup(int groupSize) {
		...
		try {
			TaskLogger.TaskDetailsArray.get(globalID).insert(0, "TaskGroup=");
	...
	TaskIDGroup(int groupSize, TaskInfo taskInfo) {
		...
		// Task Execution Logging code: start
		try {
			TaskLogger.TaskDetailsArray.get(globalID).insert(0,
					"MultiTaskGroup=");
		} catch (IndexOutOfBoundsException e) {
		}
		// Task Execution Logging code: end
	...
	public void add(TaskID<E> id) {
		//Task Execution Logging code: start
		if (paratask.logger.TaskLogger.debugFlag) {
			TaskLogger.TaskDetailsArray.get(id.globalID).append(
					", GROUP:" + globalID);
		}
		// Task Execution Logging code: end
