...
public abstract class TaskThread extends Thread {
	...
	protected boolean executeTask(TaskID task){
		...
		try {
			...
			TaskLogger.threadTaskMap.put(Thread.currentThread().getId(),
					task.globalID);
			if (TaskLogger.debugFlagMain) {
				if (TaskLogger.debugFlag) {
					TaskLogger.TaskDetailsArray.get(task.globalID).append(
							", started:" + System.currentTimeMillis()
									+ ", executing_thread:"
									+ Thread.currentThread().getName());
				} else {
					if (TaskLogger.TaskDetailsArray.get(task.globalID) != null) {
						TaskLogger.TaskDetailsArray.get(task.globalID).append(
								", started:" + System.currentTimeMillis()
										+ ", executing_thread:"
										+ Thread.currentThread().getName());
				...
				if (TaskLogger.debugFlag) {
					...
					if (TaskLogger.TaskDetailsArray.get(task.globalID) != null) {
						...
						if (task.getParentTask() != null) {
							TaskLogger.threadTaskMap.put(Thread.currentThread()
									.getId(), task.getParentTask());
						}
					}
				}
			} else {
