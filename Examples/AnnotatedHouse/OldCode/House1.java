...
public class House1 extends JApplet{
	...
	public void buildTask(Color colorWalls, Color colorRoof){
		...
		TaskInfoNoArgs<Void> foundationTask = (TaskInfoNoArgs<Void>) ParaTask.asMultiTask(()->buildAll(foundation));
		TaskID<Void> foundationID = foundationTask.start();
		
		TaskInfoNoArgs<Void> wallsTask = (TaskInfoNoArgs<Void>) ParaTask.asMultiTask(()->buildAll(wallSiding));
		wallsTask.dependsOn(foundationID);
		TaskID<Void> wallsID = wallsTask.start();
		
		TaskInfoNoArgs<Void> roofTask = (TaskInfoNoArgs<Void>) ParaTask.asMultiTask(()->buildAll(roofTiles));
		roofTask.dependsOn(wallsID);
		TaskID<Void> roofID = roofTask.start();
		
		TaskInfoNoArgs<Void> doorTask = (TaskInfoNoArgs<Void>) ParaTask.asTask(()->buildItem(door));
		doorTask.dependsOn(wallsID);
		TaskID<Void> doorID = doorTask.start();
		
		TaskInfoNoArgs<Void> windowsTask = (TaskInfoNoArgs<Void>) ParaTask.asMultiTask(()->buildAll(windows));
		windowsTask.dependsOn(wallsID);
		TaskID<Void> windowsID = windowsTask.start();
		
		TaskInfoNoArgs<Void> signTask = (TaskInfoNoArgs<Void>) ParaTask.asTask(()->buildItem(forSaleSign));
		signTask.dependsOn(roofID, doorID, windowsID);
		TaskID<Void> signID = signTask.start();
		
		try {
			signID.waitTillFinished();
