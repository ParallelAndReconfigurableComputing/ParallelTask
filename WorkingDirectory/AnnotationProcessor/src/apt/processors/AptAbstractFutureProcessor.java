...
public abstract class AptAbstractFutureProcessor {
	...
	protected List<CtStatement> getReductionStatements(String elementType, String idName){
		...
		if(!mainReductionIsUserDefinedObject){
			...
			//if the main reduction is user-defined object, then in the final reduction declaration
			//the type of the reduction is same as the name of the object (obj = obj)
			thisReductionObjectName = reductionDeclaration.getType().toString();
		}
		reductionStatements.add(getReductionSettingStatement(idName)); //ParaTask.setReduction(TaskID, reductionObj)
	...
	protected CtLocalVariable<?> createReductionDeclaration(String elementType){
		...
		createReductionPhraseForEachNestedType(); 
		String finalReductionPhrase = createTheNestedReductionPhrase(0); 
...
//----------------------------------------------------HELPER METHODS---------------------------------------------------
	...
	private CtLocalVariable<?> createMainReductionDeclaration(String mainReductionPhrase){
		...
		defaultExpression.setValue(mainReductionPhrase);
	 ...
	 */
	private void createReductionPhraseForEachNestedType(){
	 ...
	 */
	private String createTheNestedReductionPhrase(int index){
		...
		if(element.isDeclaredObject){
			...
			reductionPhrase = "new " + element.reduction + "(" + createTheNestedReductionPhrase(index+1) + ")";
	...
	private boolean isUserDefinedReductionClass(TypeElement element){
		...
		if(reductionString.contains("<")){
			generic = reductionString.substring(reductionString.indexOf("<"), reductionString.lastIndexOf(">")+1);
			reductionType = reductionString.substring(0, reductionString.indexOf("<"));
		}
		else{
			reductionType = reductionString;
		}
		Class<?> actualClass = null;
		
		try {
			actualClass = Class.forName(reductionType);			
		} catch (ClassNotFoundException e) {
			throw new IllegalArgumentException("THE CLASS " + reductionType + " SPECIFIED FOR REDUCTION DOES NOT EXIST.\n"
		...
		if(!implementsRedLibReductionInterface(actualClass)){
			System.err.println("THE CLASS: " + 	reductionType + " DOES NOT IMPLEMENT RedLib.Reduction INTERFACE!");
			return false;
		}else{
			element.genericType = generic;
			element.reduction = reductionType;
			return true;
		}		
	}
